...
public class GenericDialect extends AbstractChainedDialect {
    ...
    public <T> ITypeMapper<T> getMapper(FieldAccessor<T> fieldAccessor,
        ...
        if (targetType.isEnum()) {
            return (ITypeMapper<T>) getEnumMapper();
        }

        // Check if it's a JSON field (field-level or class-level @Json)
        if (annotationManager.getTypeInfo(fieldAccessor).getParams().containsKey(TypeParamKey.JSON)) {
            return (ITypeMapper<T>) getJsonMapper();
        }

        // Check if it's a composite type
        if (annotationManager.getTypeInfo(fieldAccessor).getParams().containsKey(TypeParamKey.COMPOSITE)) {
            return (ITypeMapper<T>) getCompositeMapper();
        }
