...
class OracleUserRepositoryTest extends OracleRepositoryTest {
        ...
        void testFindByValidated() {
                // Given — one validated user and one non-validated user
                User validated = User.builder()
                                .email("validated@example.com")
                                .validated(true)
                                .build();
                User notValidated = User.builder()
                                .email("notvalidated@example.com")
                                .validated(false)
                                .build();
                userRepository.insert(validated, "email", "validated");
                userRepository.insert(notValidated, "email", "validated");

                // When — WHERE validated = true (Boolean -> "true" in DB via TypeMapper)
                List<User> results = userRepository.findByValidated(true, "id", "email", "validated");

                // Then
                assertThat(results).hasSize(1);
                assertThat(results.get(0).getEmail()).isEqualTo("validated@example.com");
                assertThat(results.get(0).getValidated()).isTrue();
        }
