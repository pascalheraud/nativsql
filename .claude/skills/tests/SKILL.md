...
Integration tests extend `PostgresRepositoryTest`. The query methods must be implemented inside `PostgresUserRepository` (protected `newFindQuery()` / `findAll()` / `find()` / `newCountQuery()` / `count()` / `newExistsQuery()` / `exists()` etc. are only accessible from subclasses). Add a dedicated, narrowly-scoped public method to the repository for each integration test scenario (e.g. `countByStatuses(List<UserStatus>)`, `deleteByEmailAndStatus(...)`, `existsByStatuses(List<UserStatus>)`) — never add a generic passthrough that returns or accepts the protected builder type itself (`FindQuery`, `DeleteQuery`, `CountQuery`, `ExistsQuery`, ...). These builders are repository-internal implementation details and must not leak into the repository's public API, even for tests.
