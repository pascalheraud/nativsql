...
public class MySQLUserRepositoryTest extends MySQLRepositoryTest {
        @Autowired
        ...
        void testFindByCity() {
                ...
                assertThat(parisUsers).extracting(user -> user.getAddress().getCity())
                                .containsExactlyInAnyOrder("Paris", "Paris");
        }
        ...
        void testOneToManyAssociation() {
                ...
                // When
                ContactInfo found1 = contactInfoRepository.findPrimaryByUserIdAndType(userId, ContactType.EMAIL, "id",
                                "contactType", "contactValue", "isPrimary");
                ContactInfo found2 = contactInfoRepository
                                .findByUserIdAndType(userId, ContactType.PHONE, "id", "contactType", "contactValue")
                                .get(0);
        ...
        @Test
        void testInsertUserWithPassword() {
                // Given
                User user = User.builder()
                ...
                // When
                userRepository.insert(user, "email", "status", "password");

                // Then
                User found = userRepository.findByEmail("secured@example.com", "id", "email", "password");
                assertThat(found).isNotNull();
                assertThat(found.getPassword()).isNotNull();
                assertThat(found.getPassword()).doesNotContain("s3cr3tP@ssword");
                assertThat(found.getPassword()).startsWith("$2");
        }
        ...
        void testUpdateUserPassword() {
                ...
                User found = userRepository.findByEmail("pwdupdate@example.com", "id", "email", "password");
                assertThat(found).isNotNull();
                String initialHash = found.getPassword();
                assertThat(initialHash).startsWith("$2");

                // When
                found.setPassword("new@Pass2");
                userRepository.update(found, "password");

                // Then
                User updated = userRepository.findByEmail("pwdupdate@example.com", "id", "email", "password");
                assertThat(updated.getPassword()).isNotNull();
                assertThat(updated.getPassword()).startsWith("$2");
                assertThat(updated.getPassword()).isNotEqualTo(initialHash);
        }
        ...
        void testInsertUserWithNullPassword() {
                ...
                // Then
                User found = userRepository.findByEmail("nopwd@example.com", "id", "email", "password");
                assertThat(found).isNotNull();
        ...
        void testDeleteUserWithPassword() {
                // Given
                User user = User.builder()
                                .email("deletepwd@example.com")
                                .status(UserStatus.ACTIVE)
                                .password("toDelete123!")
                                .build();
                userRepository.insert(user, "email", "status", "password");

                User found = userRepository.findByEmail("deletepwd@example.com", "id", "email", "password");
                assertThat(found).isNotNull();
                assertThat(found.getPassword()).startsWith("$2");
                ...
                // Then
                User deleted = userRepository.findByEmail("deletepwd@example.com", "id", "email");
                assertThat(deleted).isNull();
        }
