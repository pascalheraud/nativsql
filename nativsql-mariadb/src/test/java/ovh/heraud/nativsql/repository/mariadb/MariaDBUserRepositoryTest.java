...
class MariaDBUserRepositoryTest extends MariaDBRepositoryTest {
        ...
        void testFindActiveUsersOrderedByFirstNameWithGetters() {
                ...
                assertThat(activeUsers).extracting(User::getFirstName).containsExactly("Alice", "Bob");
        ...
        void testFindByStatusesWithGetters() {
                ...
                // When - uses whereAndIn(User::getStatus, ...)
                List<User> found = userRepository.findByStatuses(
                                List.of(UserStatus.ACTIVE, UserStatus.SUSPENDED), "id", "firstName", "status");

                // Then
                assertThat(found).hasSize(2);
                assertThat(found).extracting(User::getStatus)
                                .containsExactlyInAnyOrder(UserStatus.ACTIVE, UserStatus.SUSPENDED);
        }
}
